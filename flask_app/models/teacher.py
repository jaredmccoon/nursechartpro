...
class Teacher:
    ...
    @classmethod
    def save(cls,data):
        query = "INSERT INTO teachers (first_name,last_name,email,password,school_id) VALUES (%(first_name)s,%(last_name)s,%(email)s,%(password)s,%(school_id)s);"
        some_id = connectToMySQL(DATABASE).query_db(query,data)
        return some_id

    @classmethod
    def get_all_by_school(cls):
        query = "SELECT * FROM teachers WHERE school_id = %(school_id)s"
        results = connectToMySQL(DATABASE).query_db(query)
        teachers = []
        for n in results:
            teachers.append( cls(n) )
        return teachers
    ...
    def validator_login(form_data):
        ...
        if len(form_data['password']) < 2:
            ...
            potential_user = Teacher.get_one_by_email({'email': form_data['email']})
